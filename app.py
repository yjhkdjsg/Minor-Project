...
def main():
    ...
    if not cap.isOpened():
        print(f"Error: Cannot open camera device {cap_device}")
        print("Please check if:")
        print("1. Your camera is connected and working")
        print("2. No other application is using the camera")
        print("3. Try a different camera device number (--device 1, 2, etc.)")
        return
    
    cap.set(cv.CAP_PROP_FRAME_WIDTH, cap_width)
    cap.set(cv.CAP_PROP_FRAME_HEIGHT, cap_height)
    print(f"Camera initialized successfully with device {cap_device}")
    print(f"Resolution set to: {cap_width}x{cap_height}")
